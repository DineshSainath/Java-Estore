...
public class eStoreSearch {                                // search class that contains search methods
    ...
    public static String runOnCommand(String[] command) {
        switch (command[0]) {
            case "add":
                String type = "";
                type = command[1];
                int id = Integer.parseInt(command[2]);
                String desc = command[3];
                int year = Integer.parseInt(command[4]);
                int price = Integer.parseInt(command[5]);
                if ((type.compareToIgnoreCase(ProductType.BOOK.toString()) == 0)) {
                    String publisher = command[6];
                    String[] authors = {};
                    authors = new String[command.length - 7];
                    for (int i = 0; i < command.length - 7; i++) {
                        authors[i] = command[7 + i];
                    }
                    if (!add(new Book(publisher, authors, id, price, year, desc), ProductType.BOOK)) {
                        return "id already added";
                    } else {
                        return "Book ID = " + id + " Added successfully";
                    }
                } else {
                    String maker = command[6];
                    if (!add(new Electronic(maker, id, price, year, desc), ProductType.ELECTRONIC)) {
                        return "id already added";
                    } else {
                        return "Electronic ID = " + id + " Added successfully";
                    }
                }
            case "search":
                try {
                ...
                if (!command[3].equals("") && !command[4].equals("")) {
                    timePeriod = command[3] + "-" + command[4];
                } else if (command[4].equals("") && !command[3].equals("")) {
                    timePeriod = "-" + command[3];
                } else if (command[3].equals("") && !command[4].equals("")) {
                    timePeriod = command[4] + "-";
                }
                ...
                if (ls.isEmpty()) {
                    return "The Search didn't give any result !";
                } else {
                    String s = "the results of the search are :\n";
                    for (Product p : ls) {
                        s += p.toString() + "\n";
                    }
                    return s;
